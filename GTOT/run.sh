...
#         python GTOT.py --pretrain_dataset $year --test_dataset $downstream_year --seed $seed
#     done
# done
...
for ((i=0; i<${#eth_years[@]}-1; i++)); do
    ...
    for seed in "${SEED[@]}"; do
        ...
        python GTOT.py --dataset eth --pretrain_dataset $current --test_dataset $next --seed $seed --epochs 100
    done
done